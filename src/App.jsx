...
const ModulePlannings = ({ defaultTab = 'cantine' }) => {
  ...
  // NOUVEAU RENDU SÉCURISÉ POUR LES ÉVÉNEMENTS DU FILTRE DE FAMILLE
  const renderFamilyEvents = () => {
    ...
    if (allEvents.length === 0) {
      return <div className="p-8 text-center text-slate-500">Aucun service planifié.</div>;
    }
                ...
                <tr className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-4">
                      <div className="flex flex-wrap items-center gap-3">
                          ...
                          {e.isError && !selectedFamilyFilter.startsWith("Mme") && <span className="ml-2 inline-flex items-center gap-1 text-[10px] font-bold bg-red-100 text-red-700 px-2 py-0.5 rounded uppercase tracking-wider border border-red-200 shadow-sm"><AlertCircle size={10}/> Jour forcé</span>}
                      </div>
                  </td>
                </tr>
              </React.Fragment>
                ...
                return (
                  <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                    <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                        <div className="flex items-center gap-4"><div className="text-xl"><FamilyPill id={selectedFamilyFilter} /></div><div><div className="text-sm text-slate-500 font-bold uppercase tracking-wider">{famInfo ? famInfo.type : ''} {famInfo && famInfo.isAsso ? '• Membre Asso' : ''}</div></div></div>
                    </div>
                    <div className="p-0">
                      {renderFamilyEvents()}
                    </div>
