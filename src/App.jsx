...
// --- FONCTION DE CALCUL (Placée en dehors pour éviter tout crash de re-rendu) ---
const generatePlanningData = () => {
  ...
  while (currentDate <= endDate) {
    ...
    let pIdx = periodsInfo.findIndex(p => dateStr >= p.start && dateStr <= p.end);
    if (pIdx !== -1 && pIdx !== currentPeriodIdx) {
      currentPeriodIdx = pIdx;
      periodThursdaysDone = false;
    }
    const activeP = periodsInfo.find(p => dateStr >= p.start && dateStr <= p.end) || periodsInfo[0];
    const isSchoolDay = !holidays.includes(dateStr) && !vacances.some(v => dateStr >= v.start && dateStr <= v.end) && [1,2,4,5].includes(dayOfWeek);
    ...
    if (isSchoolDay) {
      ...
      if (requiredParents > 0) {
        ...
        if (dayOfWeek === 4 && fauvainRiobeThursdays < 4 && !periodThursdaysDone && currentPeriodIdx < 4) {
          ...
          for (let i = 0; i < requiredParents; i++) {
            let available = familiesList.filter(f => {
              ...
              if (isInt2) {
                let hasAncien = (p1 && p1.startsWith("Mme")) || assigned.some(a => familiesList.find(fam => fam.id === a)?.type === "Ancien");
            ...
            if (available.length > 0) {
              ...
              let fallback = familiesList.filter(f => !assigned.includes(f.id)).sort((a, b) => statsObj[a.id].total - statsObj[b.id].total);
      ...
      schedule.push({ date: dateStr, period: activeP.id, dayOfWeek, p1, p2, repasType, isError: forcedError });
    }

    if (dayOfWeek === 5 && isSchoolDay) {
      ...
      if (isHolidayStart) {
        let eligibleAsso = familiesList.filter(f => f.isAsso && statsObj[f.id].menage < 4);
        if (eligibleAsso.length === 0) eligibleAsso = familiesList.filter(f => f.isAsso);
        
        if(eligibleAsso.length > 0) {
          eligibleAsso.sort((a, b) => statsObj[a.id].total - statsObj[b.id].total);
          ...
          menagesSchedule.push({ date: getDateStr(vacDate), period: activeP.id, familyId: chosen, isVacances: true, label: holidayName });
        }
      } else {
        let candidates = familiesList.filter(f => statsObj[f.id].menage === 0);
        
        if (statsObj["CHOMEL"].menage < 4) {
          if (statsObj["CHOMEL"].menage === 0) {
            if (!candidates.some(f => f.id === "CHOMEL")) candidates.push(familiesList.find(f => f.id === "CHOMEL"));
          } else {
        ...
        if (candidates.length === 0) {
          candidates = familiesList.filter(f => {
            if (f.isAsso && statsObj[f.id].menage >= 2) return false; 
            return statsObj[f.id].menage < 4; 
          });
          if(candidates.length === 0) candidates = familiesList.filter(f => statsObj[f.id].menage < 4);
          if(candidates.length === 0) candidates = [...familiesList]; 
        }
        ...
        if (candidates.length > 0) {
          ...
          statsObj[chosen].menage++; statsObj[chosen].total++; statsObj[chosen].lastMenage = dateStr;
          let nextSat = new Date(currentDate); nextSat.setDate(nextSat.getDate() + 1);
          menagesSchedule.push({ date: getDateStr(nextSat), period: activeP.id, familyId: chosen, isVacances: false, label: "Ménage Hebdomadaire" });
        }
      }
    }
  ...
  let familyStats = Object.keys(statsObj).map(id => {
  ...
  let teaCantine = (tId) => schedule.filter(s => s.p1 === tId || s.p2 === tId).length;
  familyStats.push({ id: "Mme GERARD", name: "Mme GERARD", isTeacher: true, cantine: teaCantine("Mme GERARD"), menage: 0, total: teaCantine("Mme GERARD") });
  familyStats.push({ id: "Mme SUBLET", name: "Mme SUBLET", isTeacher: true, cantine: teaCantine("Mme SUBLET"), menage: 0, total: teaCantine("Mme SUBLET") });
  familyStats.push({ id: "Mme HERVET", name: "Mme HERVET", isTeacher: true, cantine: teaCantine("Mme HERVET"), menage: 0, total: teaCantine("Mme HERVET") });

  familyStats.sort((a, b) => {
    if (a.isTeacher && !b.isTeacher) return 1;
    if (!a.isTeacher && b.isTeacher) return -1;
    return b.total - a.total || a.name.localeCompare(b.name);
  });

  return { familiesList, periodsInfo, schedule, menagesSchedule, familyStats, holidays, vacances };
};
...
const ModulePlannings = ({ defaultTab = 'cantine' }) => {
  ...
  // Initialisation sécurisée (Pas de useMemo)
  const [data] = useState(() => generatePlanningData());
    ...
    <div className="bg-transparent font-sans text-slate-800 animate-fade-in relative pb-10">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-8 print:hidden" id="main-nav">
          ...
          <button onClick={() => setActiveTab('calendrier')} className={`flex items-center gap-2 px-6 py-2.5 rounded-full font-semibold text-sm transition-all ${activeTab === 'calendrier' ? 'bg-blue-500 text-white shadow-md shadow-blue-200' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`}><Calendar size={16} /> Calendrier</button>
                    ...
                    <div key={i} className={`${dayBg} rounded-xl p-4 border ${dayBorder} shadow-sm relative hover:shadow-md transition-shadow flex flex-col`}>
                      ...
                      <div className="space-y-2 mt-auto">
                        <div className="flex items-center justify-between bg-white p-2 rounded-lg border border-slate-100 shadow-sm">
                          <span className="text-[10px] uppercase font-bold text-slate-400">P1</span><FamilyPill id={row.p1} isError={row.isError} />
                        </div>
                        <div className="flex items-center justify-between bg-white p-2 rounded-lg border border-slate-100 shadow-sm">
                          <span className="text-[10px] uppercase font-bold text-slate-400">P2</span><FamilyPill id={row.p2} isError={row.isError} />
                        </div>
                      </div>
                    </div>
                  );
                  ...
                  return (
                    <div key={i} className={`${bgCard} rounded-xl p-5 border shadow-sm relative overflow-hidden flex flex-col h-full hover:shadow-md transition-all`}>
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          <div className={`text-xs font-bold ${isVac ? 'text-amber-500' : 'text-slate-400'} uppercase tracking-wider mb-1 flex items-center gap-2`}><Calendar size={14}/> {row.label}</div>
                      ...
                      <div className="mt-auto bg-white/60 p-3 rounded-lg border border-slate-100/50 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2"><FamilyPill id={row.familyId} />{isVac && <span className="bg-amber-100 text-amber-700 text-[10px] font-bold px-1.5 py-0.5 rounded border border-amber-200 uppercase">Asso</span>}</div>
                      </div>
                    </div>
                  );
                })}
